...
class TradingStrategy:
    ...
    def _position_size(self) -> float:
        ...
        if slots_libres <= 0:
            return 0.0
        # Plafond dynamique : (capital_total_actuel / MAX_POSITIONS) × 1.20
        # Croît avec le bag, empêche une position de monopoliser le capital
        plafond  = (self.capital + sum(p["size_usdc"] for p in self.positions.values())) / MAX_POSITIONS * 1.20
    ...
    def _get_usdc_pairs(self) -> list:
        ...
        try:
            ...
            # Filtre liquidité : volume 24h >= 10M USDC
            liquid = []
    ...
    def _analyze_signal(self, symbol: str) -> dict:
        ...
        # ── Filtre stablecoin : ATR / prix < 0.10% → actif quasi-immobile ────
        if float(last1["close"]) > 0 and (atr_val / float(last1["close"]) * 100) < 0.10:
            result["details"] = f"Exclu — stablecoin/immobile (ATR/prix={atr_val/float(last1['close'])*100:.4f}%)"
            return result
    ...
    def _open_position(self, symbol: str, analysis: dict) -> str:
        ...
        return (
            ...
            f"TP1 : `{tp1_price:.6f}` (+{tp1_pct:.1f}%, vente 25%) | "
            f"TP2 : `{tp2_price:.6f}` (+{tp2_pct:.1f}%, vente 25%)\n\n"
            f"📐 *Signal :*\n{analysis['details']}"
        )
    ...
    def _check_tp(self, symbol: str, pos: dict, current_price: float) -> str:
        ...
        # TP1 — vend 25% de la position initiale
        ...
        # TP2 — vend 25% de la position initiale
            ...
            msg = (
                ...
                f"Vente 25% à `{current_price:.6f}`\n"
                f"G/P : `{pnl_u:+.2f}` USDC (`{pnl_p:+.2f}%`)\n"
                f"Reste en TS : `{pos['size_usdc']:.2f}` USDC (50% restant)"
            )
    ...
    def _close_position(self, symbol: str, price: float, reason: str) -> str:
        ...
        # Reconstruction du détail TP1 / TP2 / flottant
        ...
        # G/P total = gain déjà sécurisé aux TP + G/P de la partie clôturée maintenant
        total_pnl_usdc = secured + pnl_usdc
        total_pnl_pct  = total_pnl_usdc / size_init * 100 if size_init > 0 else 0.0

        # Reconstruction des gains TP partiels pour l'affichage
        tp1_usdc = 0.0
        tp2_usdc = 0.0
        if tp1_done and tp2_done:
            # Les deux TP ont été faits : on estime 50/50 du sécurisé
            tp1_usdc = secured * (TP1_RATIO / (TP1_RATIO + TP2_RATIO))
            tp2_usdc = secured * (TP2_RATIO / (TP1_RATIO + TP2_RATIO))
        elif tp1_done:
            tp1_usdc = secured

        tp1_pct = tp1_usdc / size_init * 100 if size_init > 0 and tp1_done else 0.0
        tp2_pct = tp2_usdc / (size_init * TP1_RATIO) * 100 if size_init > 0 and tp2_done else 0.0
        ...
        if tp1_done:
            lines.append(f"🎯 TP1 : `{tp1_usdc:+.2f}` USDC (`{tp1_pct:+.2f}%` sur investi initial)")
        if tp2_done:
            lines.append(f"🎯 TP2 : `{tp2_usdc:+.2f}` USDC (`{tp2_pct:+.2f}%` sur position post-TP1)")
        lines.append(f"📉 Clôture reste : `{pnl_usdc:+.2f}` USDC (`{pnl_pct:+.2f}%`)")
        lines.append(f"📊 *Total : `{total_pnl_usdc:+.2f}` USDC (`{total_pnl_pct:+.2f}%` sur investi initial)*")
    ...
    def scan_market_summary(self) -> str:
        """
        Analyse les paires liquides et retourne un classement Top 10
        par proximité du signal complet. Appelé à 8h, 12h, 18h quand
        ...
        lines = [
            f"📡 *Scan marché — {now_str}*",
            f"Aucune position ouverte",
            f"{regime}",
            "",
            f"*Top 10 paires les plus proches du signal :*",
        ]

        if not btc_ok:
            lines.append("⛔ Régime baissier — nouvelles entrées bloquées.")
            return "\n".join(lines)

        pairs   = self._get_usdc_pairs()[:40]   # limiter les appels API
        results = []
        ...
        for symbol in pairs:
            try:
                ...
                log.debug(f"scan_market_summary skip {symbol} : {e}")

        # Tri : signal complet d'abord, puis score décroissant
            ...
            if r["valid"]:
                ...
                for line in r["details"].split("\n"):
                    if not line.startswith("❌"):
                        continue
                    if "[1h] EMA20" in line:
                        failed.append("[1h] EMA")
                    elif "[1h] Clôture" in line:
                        failed.append("[1h] Clôture")
                    elif "[1h] Pente" in line:
                        failed.append("[1h] Pente")
                    elif "[1h] Volume" in line:
                        failed.append("[1h] Vol")
                    elif "[1h] RSI" in line:
                        failed.append("[1h] RSI")
                    elif "[4h]" in line:
                        failed.append("[4h] EMA")
                    elif "Spread" in line:
